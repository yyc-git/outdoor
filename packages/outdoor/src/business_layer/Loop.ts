...
import { InstancedLOD2, GrassInstance } from "meta3d-jiehuo-abstract"
...
export let init = (state: state, currentScene: scene, sceneNumber: number) => {
    ...
    return Device.init(getAbstractState(state)).then(abstractState => {
        ...
        return enterScene(state, currentScene, sceneNumber)
    })
}
...
let _update = (state: state) => {
    return Device.update(getAbstractState(state)).then(abstractState => {
        ...
        // .then(state => {
        //     return GrassInstance.update(state, getAbstractState, NullableUtils.getExn( state.parkScene.grass.grass2).)
        // })
}
