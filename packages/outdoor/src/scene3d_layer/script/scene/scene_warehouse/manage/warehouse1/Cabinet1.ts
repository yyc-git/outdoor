...
import { getBodyOriginName, getCabinetNumber, getOriginName,  isCabinetType } from "../../Cabinet";
...
let _enterEventHandler = (state: state, { userData }) => {
    ...
    if (!NullableUtils.isNullable(pickedWholeCabinet)) {
        ...
        if (isCabinetType(selectedCabinet.name, getType())) {
