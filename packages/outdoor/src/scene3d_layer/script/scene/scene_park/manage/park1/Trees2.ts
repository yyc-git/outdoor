...
import { Box3, DoubleSide, Group, Object3D, TextureLoader, Vector3 } from "three"
...
export let parseAndAddResources = (state: state) => {
    ...
    return ModelLoader.parseGlb(abstractState, Loader.getResource(abstractState, getTree1LowResourceId()), Render.getRenderer(abstractState)).then((low: Group) => {
        return ModelLoader.parseGlb(abstractState, Loader.getResource(abstractState, getTree1MiddleResourceId()), Render.getRenderer(abstractState)).then((middle: Group) => {
            return ModelLoader.parseGlb(abstractState, Loader.getResource(abstractState, getTree1HighResourceId()), Render.getRenderer(abstractState)).then((high: Group) => {
                ...
                // let octree = new Octree2.Octree(Terrain.getBoundingBox(NullableUtils.getExn(getState(state).terrain.terrainMesh)), 5, 0)
                let octree = new Octree2.Octree(Terrain.getBoundingBox(NullableUtils.getExn(getState(state).terrain.terrainMesh)), 2, 0)
