...
import { getDeviceState, getIsDebug, getIsProduction, getRenderState, setDeviceState, setRenderState } from "./state/State"
...
let _setSizeAndViewport = (renderer, composer, isProduction) => {
    // console.log(
...
let _bindResizeEvent = (renderer, composer, isProduction) => {
    window.addEventListener("resize", _ => {
        // camera.aspect = getWidth() / getHeight()
        // camera.updateProjectionMatrix()

        _setSizeAndViewport(renderer, composer, isProduction)
    })
}
...
export let init = (state: state, renderer) => {
    ...
    _bindResizeEvent(renderer, composer, getIsProduction(state))
...
export let render = (state: state) => {
    ...
    if (isNeedSetSize) {
        setTimeout(() => {
            _setSizeAndViewport(renderer, composer, getIsProduction(state))
        }, 100)
