...
/*! edit by meta3d */
let _removeDuplicateObject = (arr) => {
	let resultArr = [];
	let map = {}
...
class Octree {
	...
	frustumCast(intersects = [], frustum) {
		let _func = (intersects, octree) => {
			for (let i = 0; i < octree.subTrees.length; i++) {

				const subTree = octree.subTrees[i];
				if (!frustum.intersectsBox(subTree.box)) continue;

				subTree.objects.forEach(obj => {
					if (obj.geometry.boundingSphere == null) {
						obj.geometry.computeBoundingSphere()
					}

					if (frustum.intersectsSphere(obj.geometry.boundingSphere)) {
						intersects.push(obj)
					}
				})

				subTree.frustumCast(intersects, frustum)
			}
		}

		if (frustum.intersectsBox(this.box)) {
			_func(intersects, this)
		}

		return _removeDuplicateObject(intersects)
